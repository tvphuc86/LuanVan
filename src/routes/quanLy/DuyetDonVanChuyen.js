...
function DuyetDonVanChuyen() {
  ...
  const retrieveData = (filter, page, limit, ngay, thang, nam) => {
    DuyetDonVanChuyenService.getDonVanChuyen().then((rs) => {
      let data = [];
      if (ngay == 0 && thang != 0) {
        data = rs.data.filter(
          (x) =>
  ...
  const handleDuyet = (maQuanLy,maDonVanChuyen) => {
    alert(maQuanLy)
        DuyetDonVanChuyenService.duyetDon(maDonVanChuyen,maQuanLy)
        .then( rs => {
  ...
  const handleHuy = (maDonVanChuyen) => {
            if(window.confirm("Bạn thật sự muốn hủy đơn này ?")==true){
                DuyetDonVanChuyenService.huyDon(maDonVanChuyen)
                .then(rs =>{
                    if(rs.data.result)
                    {toast.success(rs.data.message)
                        retrieveData(
              ...
              <div key={i} className="don">
                ...
                <span key={i} className="">
                  <AiOutlineCheckCircle
                    ...
                    title="Duyệt đơn"
                    onClick={()=>{ handleDuyet(values.maQuanLy,e.maDonVanChuyen)}}
                  />
                  <MdCancel
                    style={{ fontSize: '30px', color: 'red' }}
                    title="Hủy đơn"
                    onClick={()=>{ handleHuy(e.maDonVanChuyen)}}
                  />
                  <MdInfoOutline
...
export default DuyetDonVanChuyen;
